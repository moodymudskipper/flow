...
especially if you are lucky enoug to have a touch screen to zoom in and out on the relevant subparts, but you might want to export your diagram
to share it or to explore it more conveniently.

This is done by using the `out` parameter of `flow_view()` and `flow_run()`.

The format will be infered from the file extension so you might do :
...
For *html* output you might want to set the `svg` parameter to `TRUE` so that you'll be able to select and copy the code.
