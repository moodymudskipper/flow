...
It and can be of length > 1 in which case all given prefixes are considered.
...
`code` is used to choose whether to display the code in code blocks or only the header, if `NA` 
the code will be displayed only if no header is defined by special comments.
...
`show_passes` is used when calling `flow_run()` to count how many time code blocks
where executed. Let's try it on the `factorial` function defined above

```{r}
flow_run(factorial(5L), show_passes = TRUE)
```

This will display a diagram even if the code fails, so we can see where it did
and what logical path we went through before the failure.
